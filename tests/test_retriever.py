"""
Tests for codecompass/retriever.py — Milestone 6: hybrid search.
...
from codecompass.retriever import (
    retrieve,
...
class TestRetrieve:
    ...
    def test_returns_at_most_return_n(self):
        many = [make_result(name=f"func_{i}", start_line=i,
                            file_path=f"/repo/{i}.py")
                for i in range(10)]
        with patch("codecompass.retriever.query_chunks", return_value=many), \
             patch("codecompass.retriever.keyword_search", return_value=[]):
            results = retrieve("query", "/fake/db", MagicMock())
        assert len(results) <= RETURN_N

    def test_returns_empty_when_both_searches_empty(self):
        with patch("codecompass.retriever.query_chunks", return_value=[]), \
             patch("codecompass.retriever.keyword_search", return_value=[]):
            results = retrieve("query", "/fake/db", MagicMock())
        assert results == []
    ...
    def test_results_have_rerank_score(self):
        raw = [make_result(name=f"func_{i}", start_line=i,
                           file_path=f"/repo/{i}.py")
               for i in range(3)]
        with patch("codecompass.retriever.query_chunks", return_value=raw), \
             patch("codecompass.retriever.keyword_search", return_value=[]):
            results = retrieve("query", "/fake/db", MagicMock())
    ...
    def test_keyword_only_results_still_returned(self):
        ...
        with patch("codecompass.retriever.query_chunks", return_value=[]), \
             patch("codecompass.retriever.keyword_search",
