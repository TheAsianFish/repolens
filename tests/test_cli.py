...
from codecompass.cli import main, _confidence_label
...
class TestIndexCommand:
    ...
    def test_index_succeeds_with_valid_repo(self, tmp_path):
        with patch("codecompass.cli.get_openai_client") as mock_client, \
             patch("codecompass.cli.index_repo",
    ...
    def test_index_shows_file_counts(self, tmp_path):
        with patch("codecompass.cli.get_openai_client"), \
             patch("codecompass.cli.index_repo",
    ...
    def test_index_missing_api_key_shows_error(self, tmp_path):
        with patch("codecompass.cli.os.getenv", return_value=None):
            runner = CliRunner()
            result = runner.invoke(main, ["index", str(tmp_path)])
...
class TestQueryCommand:
    ...
    def test_query_no_llm_flag_skips_llm(self, tmp_path):
        ...
        with patch("codecompass.cli.get_openai_client"), \
             patch("codecompass.cli.retrieve", return_value=mock_results), \
             patch("codecompass.cli.answer_query") as mock_llm:
            runner = CliRunner()
            result = runner.invoke(
                main,
    ...
    def test_query_shows_answer_and_citations(self, tmp_path):
        ...
        with patch("codecompass.cli.get_openai_client"), \
             patch("codecompass.cli.retrieve", return_value=mock_results), \
             patch("codecompass.cli.answer_query", return_value=mock_answer):
            runner = CliRunner()
            result = runner.invoke(
                main,
